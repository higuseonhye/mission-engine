  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-4">
        ...
        <Link href="/admin/clusters" className="text-sm text-[var(--accent)] hover:underline">
          Clusters & Worldview ›
        </Link>
      </div>

      <h1 className="text-2xl font-bold text-white">Admin · Missions</h1>
      <p className="mb-4 text-sm text-zinc-500">
        Add, edit, delete missions. AI Discovery suggests new ones. Supabase required for persistence.
      </p>
      <button
        type="button"
        onClick={async () => {
      ...
      <button
        ...
        className="ml-2 rounded border border-[var(--border)] px-3 py-1 text-xs text-zinc-400 hover:text-white"
      >
        Seed clusters
      </button>
      {feedback && (
        <div
          className={`ml-4 rounded px-3 py-1 text-sm ${feedback.ok ? "bg-[var(--accent)]/20 text-[var(--accent)]" : "bg-red-500/20 text-red-400"}`}
        >
          {feedback.msg}
        </div>
      )}
      ...
      <section className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-6">
        ...
        <p className="mb-4 text-sm text-zinc-400">
          AI suggests new missions based on existing ones. Approve to add.
        </p>
        <button
          type="button"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-auto rounded-xl border border-[var(--border)] bg-[var(--card)] p-6">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={() => handleSuggestUpdates(editing.id)}
                disabled={suggesting}
                className="rounded border border-[var(--border)] px-4 py-2 text-sm text-white"
              >
              ...
              <button
                type="button"
                onClick={() => loadHistory(editing.id)}
                className="rounded border border-[var(--border)] px-4 py-2 text-sm text-white"
              >
                History
              </button>
              ...
              <button
                type="button"
                onClick={() => { setEditing(null); setForm({}); setHistory([]); setHistoryMissionId(null); }}
                className="rounded border border-[var(--border)] px-4 py-2 text-sm text-white"
              >
                Close
              </button>
      ...
      <section>
        ...
        <div className="space-y-2">
          {missions.map((m) => (
            <div
              key={m.id}
              className="flex items-center justify-between gap-4 rounded-lg border border-[var(--border)] bg-[var(--card)] p-4"
            >
              <div>
              ...
              <div className="flex flex-wrap gap-2">
                {m.status !== "In Progress" && (
                  <button
                    type="button"
                    onClick={() => handleComplete(m.id, "In Progress")}
                    className="rounded border border-[var(--accent)]/50 px-2 py-1 text-xs text-[var(--accent)]"
                  >
                    In Progress
                  </button>
                )}
                {m.status !== "Solved" && (
                  <button
                    type="button"
                    onClick={() => handleComplete(m.id, "Solved")}
                    className="rounded bg-[var(--accent)] px-2 py-1 text-xs text-[var(--background)]"
                  >
                    Complete
                  </button>
                )}
                <button
                  type="button"
                  onClick={() => { setEditing(m); setForm(m); setSuggestions({}); }}
                  className="rounded border border-[var(--border)] px-3 py-1 text-sm text-white hover:border-[var(--accent)]/50"
                >
                  Edit
                </button>
              </div>
